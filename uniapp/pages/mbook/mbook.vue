<template>
	<view class="content">
		...
		<view class="cu-card case bg-white padding-bottom" :class="'no-card'">
			<view class="shadow">
				<view class="image" style="background:url('https://img0.baidu.com/it/u=3774843518,3924741707&fm=26&fmt=auto&gp=0.jpg')no-repeat center center;height: 314upx;background-size: 100% 100%;">
					<view class="flex padding-top-xl justify-center">
						<view class="content justify-center">
							<view class="text-orange text-center text-xl">{{bookVm.balance}}</view>
						</view>
					</view>
					<view class="flex padding-top-sm justify-center">
						<view class="content justify-center">
							<button class="cu-btn round line-brown" @click="selectMonth">{{query.YearMonth}} 资金结余</button>
						</view>
					</view>
					<view class="flex justify-between">
						<view class="padding-sm margin-xsradius ">
							<view class="text-olive text-center text-xl">{{bookVm.totalIncome }}</view>
							<view class="text-olive text-center text-xl">本月收入</view>
						</view>
						<view class="padding-sm margin-xsradius ">
							<view class="text-pink text-center text-xl">{{bookVm.totalExpend}}</view>
							<view class="text-pink text-center text-xl">本月支出</view>
						</view>
					</view>
				</view>
			</view>
		...
		<view v-for="(dayItem,index) in bookVm.dayBookList" :key="index">
			...
			<view class="cu-list menu-avatar">
				<view class="cu-item" :class="modalName=='move-box-'+ item.id?'move-cur':''" v-for="(item,ii) in dayItem.bookList"
				 :key="item.Id" @touchstart="ListTouchStart" @touchmove="ListTouchMove" @touchend="ListTouchEnd" :data-target="'move-box-' + item.id">
					<view class="cu-avatar round lg text-white " :class="item.classIcon" :style="'background-color:'+item.classBg">
		...
		<view class="selectPicker bg-white solid-top botton" v-if="visible">
			...
			<picker-view :indicator-style="indicatorStyle" :value="value" @change="bindDateChange" @pickstart="(e)=>{this.loadData=true;}"
			 @pickend="(e)=>{this.loadData=false;}" class="picker-view">
				<picker-view-column>
